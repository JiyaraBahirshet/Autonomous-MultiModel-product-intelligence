...
REPORT_PATH = (
    REPORT_DIR
    / "b6_3_3_exp1_gate2_test_v001.json"
)
...
def main() -> None:
    print("=" * 72)
    print("B6.3.3 EXP1 — GATE 2 FINAL TEST EVALUATION")
    print("=" * 72)
    ...
    print("VECTORIZER_FIT_DURING_EXP1_GATE2=False")
    ...
    permuted_train_transfer = (
        filtered_train_transfer[permutation]
    )

    if np.array_equal(
        permutation,
        np.arange(len(filtered_train)),
    ):
        raise ValueError(
            "Exp1 Gate 2 permutation unexpectedly identity."
        )

    print(f"PERMUTATION_SEED={SEED}")
    print(f"PERMUTATION_LENGTH={len(filtered_train)}")
    print("PERMUTATION_SCOPE=ABO_TRAIN_ONLY")

    print("VALIDATION_USED_FOR_SELECTION=False")
    print("SELECTED_GAMMA=1.0")
    print("TEST_TRANSFER_PERMUTED=False")
    ...
    print("TRAINING_EXP1_GATE2_MODEL")
